...
- 默认继承最近完成的全部辅助模块权重：`results/training/20260923-detector-1h/experiment/confidence.pt`。累计已消费区间 `[0,643275)`，接续默认从位置 643,275 开始，置信头也不重新初始化。
...
| 三个 KV 投影器 | 位置 t 符合可信条件时退出并补 KV；随后最多 64 个位置输入 teacher token、完整执行：`L_KV = mean_{anchors, s=1..H} KL(p⁹_teacher(t+s) ∥ q⁹_full_with_projected_cache(t+s))` | 每个后续位置的输出梯度都传回 P16/P20/P24，不用 KV 数值对齐损失 |
| 置信网络 C | 标签 `y = [Top-8 重合数 ≥ 6] OR [Top-1 相同]`；`L_conf = BCEWithLogits(C(h12), y)` | 在独立 gate 划分继续训练，按 dev BCE 选权重，初始权重也作为候选 |

可信规则只有 **“Top-8 至少重合 6 个，或 Top-1 相同”**；KL 保留为损失和评估指标，不再作为可信门槛。训练使用 teacher token，prompt、padding、首个生成位置不计损失。末位置不发起缺少后续监督的退出；每次投影后的监督段内不再插入新投影。段长为 `H=min(64, 当前回答/本轮数据区间剩余位置数)`，段结束才截断图，缓存值保留。
...
一个模型同时处理两个不同回答，各自缓存隔离，共用全部参数；两条回答的梯度按有效监督位置数汇总后更新一次。前 12 层与 M12/词表按每条默认 32 个位置分块，深层按位置推进；每 8 步激活重算降低显存，重算边界不截断梯度。详见 [训练实现与指标说明](experiments/training.md)。
