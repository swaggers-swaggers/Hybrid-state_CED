...
def main():
    ...
    if args.smoke:c={**c,'gate_epochs':1}
    if args.start is None:args.start=c['start_token']
    ...
    try:
        ...
        write('source_checkpoint.json',{'path':rt.source,'sha256':rt.source_sha256,'all_modules_exactly_loaded':rt.initialization,'source_interval':rt.source_interval})
        if args.verify:
            from ced_distill.verify import verify
            report=verify(rt,data)
            write('verification.json',report)
            print(json.dumps(report,indent=2));return
        expected={'confidence':('modules',),'calibrate':('confidence',),'evaluate':('modules','confidence','calibrated')}
        if args.stage in expected and (rt.loaded is None or rt.loaded['stage'] not in expected[args.stage]):raise ValueError('Wrong checkpoint stage')
        if args.stage in ('all','modules'):
            if rt.source_interval is not None and args.start!=rt.source_interval[1]:raise ValueError('Continuation must start at source interval end')
            if rt.source_interval is None and args.start!=0:raise ValueError('Nonzero continuation requires a checkpoint interval')
        stages=['modules','confidence','evaluate'] if args.stage=='all' else [args.stage]
        ...
        for stage in stages:
            ...
            if stage=='modules':
                ...
                from ced_distill.evaluation import readout_metrics,metric_delta
                before=readout_metrics(rt,data,'dev',limit);write('detector_before.json',before)
                ...
                after=readout_metrics(rt,data,'dev',limit)
                result.update(detector_before=before,detector_after=after,detector_delta=metric_delta(before,after))
