...
class DistillTests(unittest.TestCase):
    ...
    def test_top1_difference_can_be_trusted(self):
        ...
        ids,z,n=self.targets(teacher);good,overlap,kl=proximity(student,ids)
        self.assertNotEqual(teacher.argmax().item(),student.argmax().item());self.assertTrue(good.item());self.assertEqual(overlap.item(),8)
    def test_large_kl_does_not_veto_overlap(self):
        teacher=torch.arange(17.).reshape(1,17);student=teacher*4
        ids,z,n=self.targets(teacher);good,overlap,kl=proximity(student,ids)
        self.assertEqual(overlap.item(),8);self.assertGreater(sparse_kl(student,ids,z,n).item(),.05);self.assertTrue(good.item())
    def test_top1_alone_is_sufficient(self):
        teacher=torch.arange(16.).flip(0).reshape(1,16);student=-teacher.clone()
        student[0,0]=100.
        ids,z,n=self.targets(teacher);good,overlap,top1=proximity(student,ids)
        self.assertEqual(overlap.item(),1);self.assertTrue(top1.item());self.assertTrue(good.item())
    def test_six_overlap_passes_with_different_top1(self):
        teacher=torch.arange(16.).flip(0).reshape(1,16);student=teacher.clone();student[0,15]=20.;student[0,14]=19.
        ids,z,n=self.targets(teacher);good,overlap,top1=proximity(student,ids)
        self.assertEqual(overlap.item(),6);self.assertFalse(top1.item());self.assertTrue(good.item())
    def test_rejected_position_still_has_readout_gradient(self):
        teacher=torch.arange(16.).flip(0).reshape(1,16);student=(-teacher).clone().requires_grad_()
        ids,z,n=self.targets(teacher);good,_,_=proximity(student,ids)
        self.assertFalse(good.item());sparse_kl(student,ids,z,n).sum().backward()
        self.assertGreater(student.grad.norm().item(),0)
    def test_batched_and_individual_readout_gradients_match(self):
        torch.manual_seed(9);h=torch.randn(7,4);vocab=torch.randn(4,20);w=torch.randn(4,4,requires_grad=True)
        teacher=torch.randn(7,20);ids,z,n=self.targets(teacher)
        sparse_kl(h@w@vocab,ids,z,n).sum().backward();batched=w.grad.clone();w.grad=None
        for i in range(7):sparse_kl(h[i:i+1]@w@vocab,ids[i:i+1],z[i:i+1],n[i:i+1]).sum().backward()
        torch.testing.assert_close(w.grad,batched,atol=1e-5,rtol=1e-5)
